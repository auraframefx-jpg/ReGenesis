...
/**
 * Displays the ROM Tools screen, providing ROM flashing, backup/restore, system modification actions,
 * operation progress, and a dynamic Mega Man backdrop animation.
 *
 * This composable reads state from the provided [romToolsViewModel] to render device capabilities,
 * available ROMs, backups, operation progress, and action cards. It also manages file pickers for
...
/**
 * Dispatches the selected ROM action to the ViewModel to start the corresponding operation.
 *
 * FLASH_ROM and RESTORE_BACKUP are intentionally ignored here because they are handled at the
 * screen level via file pickers.
 *
 * @param actionType The ROM action selected by the user.
 * @param viewModel The ViewModel responsible for performing ROM operations.
 * @param context Android context used by the ViewModel when executing operations.
 */
